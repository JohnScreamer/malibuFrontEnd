...
const ProductInCartLabel: FC<ProductInCartLabelType> = ({ productId }) => {
    const productData = useAppSelector((state) =>
        getProductSelector(state, productId)
    );
    if (!productData?.amount) return null;
    return (
        <>
            <div className="flex gap-1">
                ...
                <Typography bold tag="span" variant="M">
                    {productData?.amount}
                </Typography>
            </div>
        </>
